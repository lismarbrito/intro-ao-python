...
if nome in alunas:
    for i, aluna in enumerate (alunas):
        if aluna == nome:
            if notas[i] < 60:
                print('A aluna ' + nome + ' tirou ' + colorama.Fore.RED + str(notas[i]))
                print(colorama.Style.RESET_ALL)
            elif notas[i] >= 90:
                print('A aluna ' + nome + ' tirou ' + colorama.Fore.GREEN + str(notas[i]))
                print(colorama.Style.RESET_ALL)
            else:
                print('A aluna ' + nome + ' tirou ' + str(notas[i]))
else:
    print(colorama.Fore.RED + 'Erro - o nome ' + nome + ' não está na lista.' + colorama.Style.RESET_ALL)
